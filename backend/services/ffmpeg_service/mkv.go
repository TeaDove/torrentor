...
func (r *Service) MKVExportWEBM(ctx context.Context, filePath string, _ int, distPath string) error {
	err := runWithErr(ctx, ffmpeg.
		Input(filePath).
		Output(distPath)) // ffmpeg.KwArgs{"c:a": "libopus"},
	// ffmpeg.KwArgs{"c:v": "libvpx-vp9", "crf": 30, "b:v": 0},
	// ffmpeg.KwArgs{"codec": "copy"},
	// ffmpeg.KwArgs{"codec:a": "ac3"},
	// ffmpeg.KwArgs{"map": []string{"0:v:0", fmt.Sprintf("0:a:%d", audioIdx)}},
...
func (r *Service) MKVExportHLS(ctx context.Context, filePath string, _ int, distDir string) error {
